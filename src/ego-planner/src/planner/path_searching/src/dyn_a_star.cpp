...
bool AStar::AstarSearch(const double step_size, Vector3d start_pt, Vector3d end_pt)
{
    ...
    {
        ROS_ERROR_THROTTLE(1.0, "Unable to handle the initial or end point, force return!");
        return false;
    }
    ...
    while (!openSet_.empty())
    {
        ...
        {
            ROS_INFO_THROTTLE(1.0, "Failed in A star path searching !!! 0.2 seconds time limit exceeded.");
            return false;
        }
    }
    ...
    if ((time_2 - time_1).toSec() > 0.1)
        ROS_INFO_THROTTLE(1.0, "Time consume in A star path finding is %.3fs, iter=%d", (time_2 - time_1).toSec(), num_iter);
