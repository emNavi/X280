...
std::string ego_drone_id="1";
namespace ego_planner
{

  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    last_remote_exp_pos << -999,-999,-999;
    ...
    nh.getParam("ego_drone_id", ego_drone_id);//载入ego的指定坐标系
    ...
    {
      waypoint_sub_ = nh.subscribe("/uav_goal", 1, &EGOReplanFSM::waypointCallback, this);
    }
    ...
    else if (target_type_ == TARGET_TYPE::REMOTE_START)
    {
      swarm_trigger_sub_ = nh.subscribe("/swarm/ego_trigger", 1,&EGOReplanFSM::swarmTriggerCallback,this);

      ROS_WARN("Waiting for trigger from MASTER");

        while (ros::ok() && (!swarm_start_trigger_ || !have_odom_))
      {
        ros::spinOnce();
        ros::Duration(0.001).sleep();
      }
  ...
  {
    have_trigger_ = true;
    cout << "PRESET_TARGET Triggered!" << endl;
    init_pt_ = odom_pos_;
  }
  void EGOReplanFSM::swarmTriggerCallback(const std_msgs::Float32MultiArray::ConstPtr &msg)
  {

    for (int i = 0; i < msg->data[0] + 1e-2; i++)
    {
        if (abs(msg->data[i + 1] - planner_manager_->pp_.drone_id) < 1e-3)
        {
            swarm_start_trigger_ = true;
        }
    }
    // have_trigger_ = true;
    // cout << "Triggered!" << endl;
  ...
  {
    if (msg->pose.position.z < -0.2){
      cout << "The target point Z is negative relative to the starting point. Please check if this coordinate is illegal!" << endl;
      // return;
    }

    cout << "MANUAL_TARGET Triggered!" << ego_drone_id << endl;
    init_pt_ = odom_pos_;
    if(msg->header.frame_id == ego_drone_id)
      {
  ...
  void EGOReplanFSM::remoteWaypointCallback(const std_msgs::Float32MultiArrayPtr &msg)
  {
    ...
    Eigen::Vector3d new_remote_exp_pos;
    new_remote_exp_pos<<msg->data[1], msg->data[2], msg->data[3];
    if(abs(msg->data[0] - planner_manager_->pp_.drone_id)<1e-2 )
    {
      if((new_remote_exp_pos - last_remote_exp_pos).norm()>0.5)
      {
        ROS_INFO("right,recv point,drone_ID %d",planner_manager_->pp_.drone_id);
        cout << "swarm_command Triggered!" << endl;
        init_pt_ = odom_pos_;
        Eigen::Vector3d end_wp(msg->data[1], msg->data[2], msg->data[3]);

        planNextWaypoint(end_wp);
        last_remote_exp_pos = new_remote_exp_pos;
      }
    }
  ...
  void EGOReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state, string pos_call)
  {
    ...
    ROS_INFO_THROTTLE(1.0, "[ %s ]: from %s to %s",
                      pos_call.c_str(),
  ...
  {
    static string state_str[8] = {"INIT", "WAIT_TARGET", "GEN_NEW_TRAJ", "REPLAN_TRAJ", "EXEC_TRAJ", "EMERGENCY_STOP", "SEQUENTIAL_START"};
    ROS_INFO_THROTTLE(1.0, "[FSM]: state: %s", state_str[int(exec_state_)].c_str());
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    if (fsm_num == 100)
    {
      printFSMExecState();
      if (!have_odom_)
        cout << "ego-planner no odom." << endl;
      if (!have_target_)
        cout << "ego-planner wait for goal or trigger." << endl;
      fsm_num = 0;
    }
    ...
    case EXEC_TRAJ:
    {
      ...
      if ((target_type_ == TARGET_TYPE::PRESET_TARGET || target_type_ == TARGET_TYPE::REMOTE_START) &&
